...
const PaymentPage = () => {
  ...
  const [payment, setPayment] = useState(); // Không có giá trị mặc định
  const [delivery, setDelivery] = useState(); // Không có giá trị mặc định
  ...
  const handleVnpayPayment = async () => {
    localStorage.setItem("vnpay_delivery", delivery);
    localStorage.setItem("vnpay_payment", "vnpay");
    localStorage.setItem("vnpay_totalPrice", totalPriceMemo);

    if (!delivery) {
      message.error(
        "Vui lòng chọn phương thức giao hàng trước khi thanh toán!"
      );
      return;
    }

    console.log("Starting VNPAY payment with delivery:", delivery);

    try {
      const res = await PaymentService.createVnpayOrder({
        token: user?.access_token,
        orderItems: order?.orderItemsSelected,
        fullName: user?.name,
        address: user?.address,
        phone: user?.phone,
        city: user?.city,
        paymentMethod: user?.paymentMethod || "vnpay",
        itemsPrice: priceMemo,
        shippingPrice: deliveryMemo,
        totalPrice: totalPriceMemo,
        user: user?.id,
        ...
        returnUrl:
          ...
          "/orderSuccess?payment=vnpay&delivery=" +
          delivery +
          "&totalPrice=" +
          totalPriceMemo,
      });
      ...
      message.error("Thanh toán VNPAY thất bại: " + error.message);
    }
  };
